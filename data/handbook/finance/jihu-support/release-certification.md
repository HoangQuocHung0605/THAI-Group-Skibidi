...
[Federal Application Security team](/handbook/security/product-security/security-platforms-architecture/application-security/).
...
[reviewed and approved by an Application Security team member](/handbook/finance/jihu-support/jihu-contribution-process/)
...
Members of the Federal AppSec team are responsible for certifying monthly releases. Their assignment is [managed by automation](/handbook/security/product-security/security-platforms-architecture/application-security/runbooks/triage-rotation/). These assignments will also need to be reflected on the
...
1. Verify that every JiHu contribution going into the release is on this list. This can be done by looking at the [status report](https://gitlab.com/gitlab-jh/status-reports/-/issues) repository information in addition to searching for the `JiHu Contribution` labels in [each repository](/handbook/finance/jihu-support/#projects) (the certification issue should have a link available). Be sure to look for both open and closed merge requests. The most likely reason a MR would be in the release but not in the checklist is the appropriate milestone had not been set for it.
...
1. Post a message in the `#security_help` Slack channel with a link to the comment
...
1. Announce in the `#security_help` Slack channel that the release cannot be certified, link to the release certification issue, and @ mention `@appsec-leadership`
