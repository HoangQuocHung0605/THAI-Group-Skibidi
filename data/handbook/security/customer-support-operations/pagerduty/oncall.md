---
title: 'Oncall'
description: 'Documentation on Customer Support Operations oncall'
date: 2026-01-07
---
...
{{% alert title="If you do not have Pagerduty access" color="warning" %}}
...
1. Click the `New Incident` button at the top-right of the page
   - Alternatively, navigate to the [Create New Incident page](https://gitlab.pagerduty.com/incidents/create)
1. Put a relevant `Title` for the incident
   - Try to be concise but descriptive
1. For `Incident Type`, select `Base Incident`
1. For `Impacted Service`, select `Customer Support Operations`
1. For `Description`, put a detailed description of the reason for the page. If there is a relevant issue link, please include it.
1. For `Urgency`, select `High`
1. For `Priority`, select `P1`
1. For `Assignee`, select `Customer Support Operations`
1. Nothing is needed in the `Advanced Options` section, so you can skip it
1. Click the `Create Incident` button at the bottom of the page
...
1. For `Urgency`, select `High`
1. For `Priority`, select `P1`
1. For `Assignee`, click the input box and click the `Users` tab
1. Re-click the input box and type the name of the specific person you are trying to page (and click the found result)
1. Nothing is needed in the `Advanced Options` section, so you can skip it
1. Click the `Create Incident` button at the bottom of the page
