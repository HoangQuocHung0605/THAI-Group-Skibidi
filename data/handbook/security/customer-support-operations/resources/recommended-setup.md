...
description: 'Documentation on team recommended setup'
date: 2026-01-07
---
...
| Visual Studios Code | You are going to need a code editor. This one aligns with the GitLab Web IDE |
...
| Zendesk US Government Token | You are going to be using the Zendesk API a good bit. Having your own admin token will be very useful |
| Zendesk US Government Sandbox Token | You are going to be using the Zendesk API a good bit. Having your own admin token will be very useful |
...
We then recommend you install the [bundler gem](https://rubygems.org/gems/bundler) on your ruby versions using the command `gem install bundler`. As most of our projects use a `Gemfile`, this means you can download the needed gems for a project by running the command `bundle`.
...
This is for macOS only. There is a Linux variant, however it does not currently work properly.
...
I begin making child folders to match the namespace pathing of all the repos I deal with.
...
I also tweaked my shell to display the branch I am working in on the prompt, mainly to help me keep track of it. This is also done in the .bashrc file (note this is going to be specifically for Linux as that is what I use):
