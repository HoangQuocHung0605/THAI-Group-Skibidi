...
description: 'Documentation on Zendesk liquid language'
date: 2025-12-23
---

This guide provides a reference for using Liquid templating language in Zendesk. Liquid enables dynamic content in triggers, automations, and themes. This page covers common variables, functions, and practical examples.

## What is the liquid language

Liquid is an [open-source template language created by Shopify](https://github.com/Shopify/liquid) and written in ruby. It is used to allow for more dynamic content on themes, pages, etc.

## How we use liquid in Zendesk

In Zendesk, we utilize liquid most commonly in triggers, automations, and extensions. We utilize this to allow for more dynamic messaging (comments, URLs, etc.). It can take time to learn how to fully utilize the liquid language, especially as it has a lot of features and is frequently updated.

## Variable interpolation

To insert a value into a text/output, you would use double curly brackets around the name of the variable/function/value.

As an example, to output a ticket requester's name, it would follow this style:

- Input: `Greetings {{ticket.requester.name}}`
- Output: `Greetings John Doe`

You can also use functions in this manner. As an example, if you wanted to put your role into a message, but ensure it was capitalized, it would follow this style:

- Input: `My role is {{current_user.role | capitalize }}`
- Output: `My role is Fullstack Engineer`
...
| Organization | `custom_fields.salesforce_id` | The Salesforce ID for an organization |
...
### Determining ticket field IDs

This often can require admin level access, so it is best to reach out to the Customer Support Operations team for assistance.
...
| [url_decode](https://shopify.github.io/liquid/filters/url_decode/) | Decodes a string that was encoded by url_encode |
...
### Determine if we should show an out-of-hours message

We used this to determine if a block of text should display based on if the ticket is currently within business hours or not.
