...
- GitLab PSIRT Security Engineering
...
- Guiding principles for handling reports:
  ...
  - Medium/Low reports are handled through automation
  - PSIRT engineers on rotation should ensure proper assignment and triage
...
- When the `GitLab Team` queue is empty, regularly check that the `H1 Triage` queue doesn't contain reports that are rated as `Critical` or `High`. If there are such rated reports, evaluate if they are indeed `Critical` or `High`, and if so handle them directly without waiting on `H1 Triage`.
  ...
  - If a `Critical` report in the `New` queue has been evaluated by a PSIRT engineer to be of lower severity, enter a comment in the report to `Team only` detailing why this report does not appear to be `Critical`. This will inform the other team members that it has already been quickly reviewed to avoid duplication of work. Be sure to leave the report in the `New` queue and leave a note to inform the HackerOne triage team to continue triaging the report.
- The PSIRT engineer on rotation should make every effort to ensure that _all_ the reports that are assigned to `GitLab Team` within their triage week are both assigned (to themselves) and properly triaged.
...
- When a report contains externally-hosted static content for reproduction (for example some HTML file triggering a CSRF or a vulnerability exploiting a `postMessage` issue), follow the instructions in [this project](https://gitlab.com/gitlab-com/gl-security/product-security/appsec/vuln-repro-static-pages) to re-host it internally
  - Do not use a HackerOne reporter's PoC. Develop an exploit payload that will test the vulnerability while eliminating the risk of a malicious payload.
...
the responsible engineering team:
  ...
  - Update the CVE issue and Bug Bounty Council note with relevant details, while they are still fresh in your mind
    ...
    - Input _only_ the bounty amount in "Suggested Bounty" row - any/all additional details for this bounty award amount should go in "Notes"
...
- Identify the most appropriate [non-CVSS bounty amount](https://gitlab-com.gitlab.io/gl-security/product-security/appsec/cvss-calculator/) and add your initial [suggested bounty](https://docs.hackerone.com/en/articles/8524543-bounties#h_13d3d2c2b7) in H1
- Use `/h1 bounty REPORT_ID` to create a comment on the Bug Bounty Council issue (this step should not be necessary if `/h1 import` was previously run without the `~no-bounty` option.
- Support SIRT as required and, if applicable, follow the process for [handling severity::1/priority::1 issues](/handbook/security/product-security/psirt/runbooks/handling-s1p1/)
- Investigate the location of the exposure, and locations like it, for further exposure.
  - Check the history on issue / MR descriptions
  - Check git commit history
  - Check build artifacts
...
**Note:** Features behind feature flags are often in Experimental or Beta stages. See [Triaging vulnerabilities in Experimental and Beta features](#triaging-vulnerabilities-in-experimental-and-beta-features) for guidance on CVE assignment and blog post inclusion for these features.
...
### Triaging vulnerabilities in Experimental and Beta features

GitLab does not issue CVE identifiers for vulnerabilities discovered in features that are in Experimental or Beta stages of development. These features are explicitly not ready for production use, require opt-in enablement, and do not follow standard vulnerability remediation SLOs.

**Determining feature maturity stage:** To identify a feature's maturity stage, search for the feature in the [GitLab documentation](https://docs.gitlab.com/). Features in Experimental or Beta stages will display a status banner at the top of their documentation page (for example, see [GitLab Observability](https://docs.gitlab.com/operations/observability/)). If no status banner is present, the feature is considered Generally Available.

When triaging reports affecting Experimental or Beta features:

- Import the report with the `~no-cve` flag: `/h1 import <report> [project] ~no-cve`
- Follow normal triage processes for severity assessment and remediation
- The vulnerability will not be included in security release blog posts
- Standard remediation SLOs do not apply to these features
- Security fixes may be released in canonical (in the open) for features that are in Experimental or Beta status.

**Rationale:** Experimental and Beta features lack the formal versioning, support commitments, and production-readiness required for CVE Records. Users who opt into these features accept the associated risks as documented in our [feature maturity policy](https://docs.gitlab.com/policy/development_stages_support/).

For more details on maturity stages requirements, including around security, consult the [Development Stages Support policy](https://docs.gitlab.com/policy/development_stages_support/).
...
- Award approval:
  ...
  - If needed, make a request in `#sec-appsec-team-only` when your note has not received enough votes
...
We can award [Contributor Success store](https://rewards.gitlab.com/) credits to reporters who have submitted a quality report that did not qualify for a monetary reward in our Bug Bounty program.
To award credits, please reach out to the Contributor Success team on Slack.

Reward credits do not have a monetary value, and can also be used to plant trees in the GitLab Forest. This is helpful where reporters for various reasons can't accept swag or a monetary award. The number of credits varies based on the severity, please indicate this when reaching out to Contributor Success.
...
### PSIRT Security Engineer Procedures for severity::1/priority::1 Issues

Please see [Handling severity::1/priority::1 Issues](/handbook/security/product-security/psirt/runbooks/handling-s1p1/)

### PSIRT Security Engineer Procedures for Medium/Low severity issues

Reports that have been triaged by HackerOne, scored as Medium/Low severity, and assigned to the `GitLab Team` are automatically imported as an issue. The respective EM/PM for the vulnerable product feature documented in the report are notified in a comment.

The automation assigns a PSIRT engineer to the council issue as 'imported by [engineer name],' establishing their ownership and accountability for the issue. The assigned engineer is responsible for verifying the CVSS score and taking appropriate action as needed. PSIRT engineers should regularly review their to-do lists to monitor incoming assignments.

Inaccuracies in issues created by the automation are tracked by applying the following labels:

- If an imported GitLab issue is assigned to the wrong product team/DRI, apply the label `H1-Assignment::Incorrect`
- If an imported GitLab issue is determined to be a false-positive, apply the label `H1-Status::FalsePositive`
- If an imported GitLab issue is determined to be out of scope from the Bug Bounty program, apply the label `H1-Policy::OOS`
- If a report requires PSIRT intervention after being imported, apply the label `H1-Escalation::PSIRT`
...
If PSIRT cannot reproduce an issue, it is unlikely that the product team will be able to either. Without steps to reliably reproduce a vulnerability, it will be difficult to determine the root cause, propose a meaningful fix, or verify whether a solution will fully address the issue.
...
- Visit the [Zendesk portal](https://gitlab-internal.zendesk.com/hc/en-us/requests/new?ticket_form_id=22783840298780)
  - For `Choose the reason why you are reaching out to us today` keep the **L&R Internal Request** default option
  - For `What category of request?` first select **Other** then select **Hacker One Reporter license**
  - For `What expiration date should be used?` select the date that's one calendar year away from the requested date
  - For `What is the contact's name?` use "HackerOne analysts"
  - For `What is the contact's email?` use `analysts@h1triage.com`

- In the description, paste the following:
