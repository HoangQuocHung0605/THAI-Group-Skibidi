...
US Government Support assigns tickets to Support Engineers using a [weighted Round Robin algorithm](#ticket-assignment-via-round-robin).
...
After 7 days of a case being in the `pending` state an automation will run that notifies the requestor that we are waiting on their response. After 7 more days in the `pending` state an automation will run to move the case to a `solved` state.
...
Requests for Help live "In-Boundary" on [CompSecGov](https://compsecgov.gitlab-dedicated.us/gitlab-dedicated-us-public-sector/customer-support). Access to CompSecGov comes through [FedRAMP Okta](https://gitlabus.okta.com). If you need, but don't have, access contact Wade or Kasey to get the process started.  
