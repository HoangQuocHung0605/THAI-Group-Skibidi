...
- Respond to new emergency requests by helping the on-call engineer [triage the emergency request](/handbook/support/workflows/customer_emergencies_workflows/#triage-the-emergency-request).
...
In case an Account Escalation opens where support involvement is needed, it is the responsibility of the support manager on-call to find a Lead Support Manager. In case this is an ASE account (you can find this in the org notes) please ping Lissa Roberts (AMER), Ilia Kosenko (EMEA) or Wei Meng Lee (APAC).
...
This feedback form creates issues in the customer feedback project, with a subject format of **Positive/Negative/Neutral feedback for ticket nnnnnn**, and is automatically assigned to the manager of the engineer assigned to the ticket, or to Val Parsons as Support Director when there is no assignee. In addition to the issue, there is a Slack notification sent to the #support_ticket-attention-requests channel.
The following action should be taken promptly by the Support Manager On-call:

If the feedback related to an existing ticket:

1. Review feedback and ticket information and consider how urgent a reply is required. Replying as soon as possible could prevent further customer frustration and a STAR or emergency situation.
1. Keeping urgency in mind, decide if action should be taken by the manager of the assigned engineer or by the Manager On-Call.
1. If the feedback needs immediate attention, reply via the ticket or email, as appropriate, share next steps and offer your Calendly link to schedule a video call.
...
If there is no ticket link and it is general support feedback:

1. The issue will automatically be assigned to Val Parsons - please re-assign it to yourself as DRI.
1. Review feedback and consider next best action.
1. If customer would like a call, offer your Calendly link to schedule a video call.
1. If the feedback is for Product or other teams, share it in the appropriate channel.
1. If there are any additional actions that arise from your interaction with the customer, go back and note them in the Feedback Issue.
