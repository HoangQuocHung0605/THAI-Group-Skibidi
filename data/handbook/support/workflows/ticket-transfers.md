...
1. Proceed with all appropriate initial steps to begin addressing the customer's
   issue
   - **Please review your initial response before sending it, and ask yourself
       if it is what you would want to see if you were receiving the rehome.**
1. Inform the customer that before the end of your workday you will transfer
   ...
   - NOTE: APAC may also maintain assignment of low priority tickets with a preferred region of AMER. See [the original policy announcement](https://gitlab.com/gitlab-com/support/support-team-meta/-/work_items/6251) for context.
1. If it's the end of your work day and the customer has **not** requested that
   you keep the ticket, proceed to the next section to initiate a rehome.
1. Otherwise, follow the instructions for
   [Keeping an out-of-region ticket](#keeping-an-out-of-region-ticket)
...
If the ticket is waiting on a customer reply:

1. set the ticket status to **Pending**
1. the ticket will remain assigned to you until the customer responds, at which
   point:
   1. Zendesk will set the ticket status to **Open**
   1. a trigger will run to remove ticket assignment so the ticket will become
      visible in the global view
1. submit the ticket to initiate the rehome

Otherwise, if the ticket is waiting on an update from a support engineer:

1. set the ticket status to **Open**
1. in the `Assignee` field, select the option to assign the ticket to your
   region's group; for example, select the option **Assign to Support AMER**
   - this is the way in Zendesk that an agent can cause ticket assignment to
     be removed
1. submit the ticket to initiate the rehome
...
1. Use the `Support::Rehome::Complete Rehome` macro, which:
   ...
   1. Removes the `rehome_initiated` tag
...
**Only** if a customer responds to your initial update on the ticket with an
explicit request that you keep it rather than transfer it to their preferred
region, should you follow these steps:
