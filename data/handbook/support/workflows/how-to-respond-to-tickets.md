...
The second mode of operation is the Hypothesis Testing mode. This is the creative step where we can behave like scientists and theorize about what might be happening on the user's side.
...
we can't promise to cover. This is a tool to set expectations for customers as well as to help the
...
By taking a [docs-first](https://docs.gitlab.com/development/documentation/styleguide/#docs-first-methodology) approach to answering, we can ensure that the documentation remains a highly useful [single source of truth](https://docs.gitlab.com/development/documentation/styleguide/#documentation-is-the-single-source-of-truth-ssot). By building up a corpus of documentation informed by real-world problems, we help GitLab customers find the answers or solutions they need before they come into the queues. Our **Knowledge base** is growing and thriving - with over 300 knowledge articles now published and consistently high viewership, we're seeing the real impact of documentation efforts. These articles are becoming the go-to resource for both customers and team members alike.

**Always respond with a link to the docs or a relevant knowledge article. If docs content is missing, create it and provide the customer with a link to the MR. If knowledge articles could address common questions, create them. If you're working on a ticket about to breach, clear the breach with your response, then subsequently follow up with the MR or knowledge article. Remember: go slow to go fast.**
